...
const registerCart = asyncHandler(async (req :Request,  res :Response) => {
    const user = req.user._id;
...
const addToCart = asyncHandler(async (req :Request,  res :Response) => {
    ...
    const matchingOption = product.options.find(
        (option) =>
            ...
            option.sizes.some((size) chat=> size.size === sizes.size)
    );
...
export {
    ...
    getAllCarts, getCart, getCartById, registerCart
};
